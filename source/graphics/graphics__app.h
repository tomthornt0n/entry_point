...
#define G_WindowsForEach(V) W_Handle (V) = G_WindowsFirstGet(); NULL != (V); (V) = G_WindowsNextGet(V)

Function Bool *G_ShouldBlockToWaitForEvents (void); // NOTE(tbt): read/write access to a bool to control whether the thread should block to wait for window message
